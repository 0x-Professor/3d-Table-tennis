...
export default function StylizedPlayerPaddle() {
  ...
  useFrame((state, delta) => {
    ...
    const newZ = Math.max(0.5, Math.min(1.8, position.z + velocity.z * delta))

    paddleRef.current.setTranslation({ x: newX, y: 0.3, z: newZ }, true)

    dispatch({
      type: "UPDATE_PLAYER_PADDLE",
      position: [newX, 0.3, newZ],
    })
  })
